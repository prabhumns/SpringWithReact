...
public class GrpcServer extends SpringWithReactGrpc.SpringWithReactImplBase {
	...
	@Override
	public StreamObserver<HelloRequest> streamHello(
		StreamObserver<HelloReply> responseObserver
	) {
		return new StreamObserver<HelloRequest>() {
			...
			public void onNext(final HelloRequest request) {
				responseObserver.onNext(
					HelloReply.newBuilder()
						.setMessage(
							"Hello World! %s".formatted(request.getName())
						)
						.build()
				);
			}
			...
			public void onError(final Throwable throwable) {
				...
				responseObserver.onNext(
					HelloReply.newBuilder()
						.setMessage(
							"Received an exception %s".formatted(
			...
			public void onCompleted() {
				responseObserver.onNext(
					HelloReply.newBuilder().setMessage("About to close").build()
				);
				responseObserver.onCompleted();
			}
		};
