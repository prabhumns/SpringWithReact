...
public class HelloRestController {
	...
	private final GRPCBinGrpc.GRPCBinBlockingV2Stub grpcbin =
		GRPCBinGrpc.newBlockingV2Stub(
			ManagedChannelBuilder.forAddress("grpcb.in", 443)
	...
	public HelloResponse hello() {
		final var response = springWithReact.sayHello(
		...
		final var grpcResponse = grpcbin.dummyUnary(
			Grpcbin.DummyMessage.newBuilder().build()
		);
		log.info("Message from GRPC: {}", grpcResponse);

		return new HelloResponse(
			response.getMessage() + grpcResponse.getFString()
		);
	}
}
