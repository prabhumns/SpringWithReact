...
public class SubscriptionService {
	...
	> stockNameToSessionIds = new ConcurrentHashMap<>();
	private final Map<String, SseEmitter> sessionIdToEmitter =
	...
	public void addListener(
		...
		stockNames.forEach(stockName -> {
			stockNameToSessionIds.compute(stockName, (key, value) -> {
				if (value == null) {
	...
	public void broadcast(final String stockName, final Double value) {
		final var sessionIdsOfStock = stockNameToSessionIds.get(stockName);
		if (sessionIdsOfStock == null) {
			log.info("{} stock is not subscribed.", stockName);
			return;
		}
		log.debug(
			"Found session ids {} for staock {}",
			String.join(",", sessionIdsOfStock),
			stockName
		);
			...
			.map(sessionId -> {
				...
				if (emitter == null) {
					...
					try {
						...
						log.error(
							"IO Exception while sending message to emitter for session id {}",
							sessionId,
							e
						);
						...
						log.error(
							"The session seem to be closed hence removing the session id {}",
							sessionId,
							e
						);
						return sessionId;
	...
	public void sendRandomEvent() {
		this.broadcast(
			String.valueOf((char) (secureRandom.nextInt(3) + 'A')),
			secureRandom.nextDouble(10, 100)
		);
	}
